...
void draw_block(int x){
	int i;
  for (i=0;i<7;i++){
		set_pixel(i,x);
	}
}
void clear_block(int x){
	int i;
  for (i=0;i<7;i++){
		clear_pixel(i,x);
	}
...
void display_image(int x, const uint8_t *data) {
	...
	for(i = 0; i < 4; i++) {
		...
		for(j = 0; j < 128; j++) //gamla värdet j= 32, i*32
